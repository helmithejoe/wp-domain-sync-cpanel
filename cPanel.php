...
class Domain
{
  ...
  * Returns true on success or false on failure.
  * @return array
  * added by helmi@bee38.net
  */
	function listParkDomain()
	{
		//itemsperpage=1000 to make sure that all domains are loaded
		$url = 'park/index.html?api2_paginate_start=1&api2_sort_column=&api2_sort_reverse=0&itemsperpage=1000&page=1&searchregex=&skip=-1';
		
		$r = $this->HTTP->getData($url);
		$tmp = explode('<tbody>', $r);
		$tbody = $tmp[1];
		$tmp = explode('<td class="nobrd-center-t">', $tbody);
		//print_r($tmp);
		$domain_arr = array();
		for($i=1;$i<count($tmp);$i++) {
			$tmp2 = explode('<td nowrap="nowrap">', $tmp[$i]);
			$domain_arr[] = trim(substr($tmp2[0], 0, strpos($tmp2[0], '</td>')));
		}
		return $domain_arr;
	}
  ...
  * check login
  *
  * Returns int on success or false on failure.
  * @return array
  * added by helmi@bee38.net
  */
	function checkLogin()
	{
		...
		return strpos($r, 'doaddparked.html');
	}
